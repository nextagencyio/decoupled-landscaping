...
export default async function GenericPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  try {
    ...
    if (!entity) {
      return (<div className="min-h-screen bg-stone-50"><Header /><main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12"><PageNotFound path={path} /></main></div>)
    }
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 pt-24">
          <ErrorBoundary>
            <article className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              {image && <ResponsiveImage image={image} alt={image.alt || title} className="rounded-t-2xl" priority={true} />}
